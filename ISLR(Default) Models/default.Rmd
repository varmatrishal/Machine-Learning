...
# Step 1 
Setting up the data Default using the ISLR library. Then running dim and names to get more informatoin. 
Seed funciton to replicate the same data, and then dividing into test and train. 
```{r}
#a
library(ISLR)
data(Default)
...
# Step 2 
Creating a logistical regression model on the training data where AHD is predicted by all other variabled. Computed the accuracy. 
```{r}
#a. 
...
```
# Step 5
Cross validation funcition used to get the best n value and ploteed on a 1x2 format. 
```{r}
#a 
