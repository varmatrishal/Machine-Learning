...
## Problem 1 step 3 
evaulating the model for test data. 
```{r}
...
b. In terms of mse, it was the oppposite. mse for kNN model was 20.071, whiel linear regression had 21.75. Although it is not a huge difference. 
 
c. kNN performas better when the data is scaled. That is the reason why it was different in these cases. 
...
using labels to create the mdoel. 
```{r}
library(class)
...
Running kNN predictor on column 2-8, and 8 to 10 using k=1 as defauly. Listing the accuracy of the data. 
```{r}
library(class)
...
a. We had accuracy of 89.14% when we evaluated the model and used the confusion matrix. When we used the kNN function, with Class as target, we had an accuracy of 78%, and with specefic columns in step 5, we had a lowered accuracy of 72.13%. 

b. kNN is considered the lazy learning function. As we narraowed the data, the prediction became worse and worse, as it is able to determine higher percentage when it has more data to use in its function. kNN performs poorly in low dimnesion. 
